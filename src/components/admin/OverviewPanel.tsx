...
export default function OverviewPanel({ userEmail }: OverviewPanelProps) {
  const stats = [
    { label: "Total Produk", value: "4", icon: Package, color: "text-blue-500 dark:text-blue-400", bg: "bg-blue-500/10 border-blue-500/20" },
    { label: "Pesanan Masuk", value: "3", icon: ShoppingCart, color: "text-amber-500 dark:text-amber-400", bg: "bg-amber-500/10 border-amber-500/20" },
    { label: "Total Pelanggan", value: "3", icon: Users, color: "text-emerald-500 dark:text-emerald-400", bg: "bg-emerald-500/10 border-emerald-500/20" },
    { label: "Revenue (Simulasi)", value: "Rp 259 Jt", icon: TrendingUp, color: "text-red-500 dark:text-red-400", bg: "bg-red-500/10 border-red-500/20" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 p-4 rounded-xl border border-red-500/20 bg-red-500/5">
        ...
        <div>
          <p className="text-sm font-bold text-foreground">Panel Administrator IKN</p>
          <p className="text-xs text-muted font-mono">Login sebagai: {userEmail}</p>
        </div>
      </div>
        ...
        {stats.map((stat, i) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <p className="text-[10px] text-muted font-mono">{stat.label}</p>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody>
              {MOCK_ORDERS.map(order => (
                <tr key={order.id} className="border-b border-border/40 hover:bg-elevated/40 transition">
