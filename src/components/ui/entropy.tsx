...
export function Entropy({ className = "", size = 400, particleColor }: EntropyProps) {
  ...
  useEffect(() => {
    ...
    // Determine particle color dynamically based on active theme or prop
    const isDark = document.documentElement.classList.contains('dark')
    const finalParticleColor = particleColor || (isDark ? '#ffffff' : '#000000')
    ...
    class Particle {
      ...
      draw(ctx: CanvasRenderingContext2D) {
        const alpha = this.order ?
          ...
          0.8
        ctx.fillStyle = `${finalParticleColor}${Math.round(alpha * 255).toString(16).padStart(2, '0')}`
        ctx.beginPath()
    ...
    function animate() {
      ...
      particles.forEach(particle => {
        ...
        particle.neighbors.forEach(neighbor => {
          ...
          if (distance < 50) {
            ...
            ctx.strokeStyle = `${finalParticleColor}${Math.round(alpha * 255).toString(16).padStart(2, '0')}`
            ctx.beginPath()
      ...
      // 添加分隔线和文字
      ctx.strokeStyle = `${finalParticleColor}4D`
      ctx.lineWidth = 0.5
      ctx.beginPath()
      ctx.moveTo(size / 2, 0)
