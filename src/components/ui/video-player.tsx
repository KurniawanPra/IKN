...
const VideoPlayer = ({ src }: { src: string }) => {
  ...
  return (
    <motion.div
      className="relative w-full max-w-4xl mx-auto rounded-xl overflow-hidden bg-[#11111198] shadow-[0_0_20px_rgba(0,0,0,0.2)] backdrop-blur-sm aspect-video flex items-center justify-center"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5 }}
      onMouseEnter={() => !isYouTube && setShowControls(true)}
      onMouseLeave={() => !isYouTube && setShowControls(false)}
    >
      {isYouTube ? (
        <iframe
          src={embedUrl}
