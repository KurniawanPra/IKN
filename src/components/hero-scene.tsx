...
function CentralShape() {
  ...
  // Shared material for the main logo mesh to animate its properties efficiently
  ...
  useFrame(({ clock }) => {
    ...
    const scale = 1 + Math.sin(t * 1.2) * 0.025;
    logoGroupRef.current.scale.set(scale, scale, scale);

    // Opposing rotation for the outer metallic rings (gyroscope effect)
  ...
  return (
    <group>
      ...
      <group ref={logoGroupRef}>
        {/* Triangular Frame */}
        <mesh geometry={triangleGeometry} position={[0, 0, -0.09]} material={logoMaterial} />

        {/* Bottom Rectangle Frame */}
        <mesh geometry={rectGeometry} position={[0, 0, -0.09]} material={logoMaterial} />
      ...
      <mesh ref={ring2Ref}>
        <torusGeometry args={[2.45, 0.03, 16, 120]} />
        <meshStandardMaterial
          color="#a63030"
          roughness={0.15}
          metalness={0.95}
...
function Scene() {
  ...
  useFrame(() => {
    // Responsive, high-fidelity cursor tracking rotation
